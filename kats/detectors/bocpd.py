...
class BOCPDModelParameters(ABC):
    ...
    def set_prior(self, param_dict: Dict[str, float]) -> None:
...
class BOCPDetector(Detector):
    ...
    available_models: List[BOCPDModelType]
    ...
    def __init__(self, data: TimeSeriesData) -> None:
        ...
        self.change_prob: Dict[str, ArrayLike] = {}
    ...
    ) -> Tuple[float, BOCPDModelParameters]:
...
class _BayesOnlineChangePoint(Detector):
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        if rt_posterior is None:
            raise ValueError("Call detect() before plot().")
        assert next_pred_prob is not None
        ...
        for ts_ix, ts_name in enumerate(ts_names):
            ...
            if self.debug:
                pred_mean_arr = self.pred_mean_arr
                pred_std_arr = self.pred_std_arr
            ...
            if self.debug:
                ...
                plt.plot(
                    ...
                    next_pred_prob[lag + 1 : self.T, lag, ts_ix],
                    "k-",
                )
                plt.xlabel("Time")
...
def check_data(data: TimeSeriesData) -> None:
...
class _NormalKnownPrec(_PredictiveModel):
    ...
    def __init__(self, data: TimeSeriesData, parameters: NormalKnownParameters) -> None:
        ...
        self._maxT: int = len(data)

        # hyper parameters for mean and precision
        self.mu_0: Optional[float] = mean_prior
        self.lambda_0: Optional[float] = mean_prec_prior
        self.lambda_val: Optional[float] = known_prec
        if data.is_univariate():
            self._data_shape = self._maxT
        else:
            # Multivariate
            self.P: int = data.value.values.shape[1]
    ...
    def _find_empirical_prior(self, data: TimeSeriesData) -> None:
        """
...
class _BayesianLinReg(_PredictiveModel):
    ...
    def _residual_variance(
        ...
        predictions = intercept + slope * np.array(x)
        residuals = predictions - np.array(y)
    ...
    def pred_std(self, t: int, x: float) -> npt.NDArray:
        ...
        return np.asarray(self._std_arr[t])
